CLASS zcl_abapgit_log_util DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE.
...
CLASS zcl_abapgit_log_util IMPLEMENTATION.
  ...
  METHOD write_application_log.
    ...
    IF iv_log_handle IS INITIAL.
      zcx_abapgit_exception=>raise( 'Log handle is required for logging' ).
    ENDIF.
    ...
    IF sy-subrc <> 0.
      " If logging fails, don't raise exception to avoid breaking main process
      RETURN.
    ENDIF.
