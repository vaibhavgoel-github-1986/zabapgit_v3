...
CLASS ZCL_ABAPGIT_GUI_PAGE_REPO_OVER IMPLEMENTATION.
  ...
  METHOD prepare_overviews.
    ...
    " Restrict visibility for non-admin users: keep only configured repo name
    IF zcl_abapgit_utils=>is_admin( ) = abap_false.
      DELETE rt_overviews WHERE name <> 'sap-brim-github-repo'.
    ENDIF.
  ...
  METHOD render_action_toolbar.
    ...
    IF zcl_abapgit_utils=>is_admin( ) = abap_false.
      " For non-admins show only Stage by Transport; remove selection dependent CSS classes so it's always visible
      lo_toolbar->add(
...
*    IF zcl_abapgit_utils=>is_admin( ) = abap_true.
    ii_html->add( |<tr{ lv_fav_tr_class } data-key="{ is_repo-key }" data-offline="{ is_repo-offline }">| ).
*    ELSE.
*      " Omit data-key so row is not treated as selectable/openable for non-admin
*      ii_html->add( |<tr{ lv_fav_tr_class } data-offline="{ is_repo-offline }">| ).
*    ENDIF.
...
*        iv_interactive = zcl_abapgit_utils=>is_admin( ) ) ).
    ...
    ELSE.
      ii_html->td( iv_class = 'ro-go wmin' ).
    ENDIF.
  ...
  METHOD zif_abapgit_gui_event_handler~on_event.
    ...
    CASE ii_event->mv_action.
      WHEN c_action-select.
        ...
        IF zcl_abapgit_utils=>is_admin( ) = abap_false.
          " Non-admins not allowed to open repository view
